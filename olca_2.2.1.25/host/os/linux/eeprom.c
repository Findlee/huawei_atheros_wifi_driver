...
/* WIFI module porting, hanshirong, begin */
    ...
    {
	    /* convert mac address */
	    AR_DEBUG_PRINTF("%s: mac_param is %s\n", __FUNCTION__, mac_param);
	    if (!wmic_ether_aton(mac_param, mac_addr)) {
		    printk("%s: convert mac value fail\n", __FUNCTION__);
		    goto soft_mac_end;
	    }
	    
	    /* the converted mac address */
	    AR_DEBUG_PRINTF("%s: the converted mac value is", __FUNCTION__);
	    for (i = 0; i < ATH_MAC_LEN; i++)
		    AR_DEBUG_PRINTF("[0x%x],", mac_addr[i]);
	    AR_DEBUG_PRINTF("\n");
	    ...
	    /* mac address in eeprom array */
	    AR_DEBUG_PRINTF("%s: mac address in eeprom array is ", __FUNCTION__);
	    for (i = 10; i < 10 + 6; i++)
		    AR_DEBUG_PRINTF("[0x%x],", eeprom_data[i]);
	    AR_DEBUG_PRINTF("\n");
    }
